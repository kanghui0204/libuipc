...
namespace
{
    ...
    // functors replacing the device lambdas passed to FastSegmentalReduce
    struct ExternalArticulationConstraintGetKey
    {
        cuda_tool::CBufferView<Vector2i> joint_joint_id_to_joint_ij;

        __device__ IndexT operator()(IndexT I) const
        {
    ...
    struct ExternalArticulationConstraintGetValue
    {
        cuda_tool::CBufferView<Float>    joint_joint_id_to_mass;
        cuda_tool::CBufferView<Vector2i> joint_joint_id_to_joint_ij;
        cuda_tool::CBufferView<Vector6>  joint_id_to_L_basis;
        cuda_tool::CBufferView<Vector6>  joint_id_to_R_basis;
        cuda_tool::CBufferView<Vector2i> joint_id_to_body_ids;
        cuda_tool::CBufferView<Float>    joint_id_to_delta_theta_tilde;
        cuda_tool::CBufferView<Vector12> qs;
        cuda_tool::CBufferView<Vector12> q_prevs;
        cuda_tool::CBufferView<Vector2i> joint_id_to_ref_q_prev_ids;
        cuda_tool::BufferView<Vector12>  ref_q_prevs;
        cuda_tool::CBufferView<U64>      joint_id_to_uid;
        U64                              RevoluteUID;
        U64                              PrismaticUID;

        __device__ Float operator()(IndexT joint_jointI) const
        {
    ...
    __global__ void external_articulation_constraint_compute_energy_kernel(
    ...
    __global__ void external_articulation_constraint_compute_gradient_hessian_k1_kernel(
        ...
        cuda_tool::CBufferView<Float>           joint_id_to_G_theta,
        cuda_tool::CBufferView<Vector2i>        joint_id_to_body_ids,
        cuda_tool::CBufferView<Vector6>         joint_id_to_L_basis,
        cuda_tool::CBufferView<Vector6>         joint_id_to_R_basis,
        cuda_tool::CBufferView<Vector12>        qs,
        ...
        int                                     n)
    {
        int jointI = blockIdx.x * blockDim.x + threadIdx.x;
        if(jointI >= n)
            return;
    ...
    __global__ void external_articulation_constraint_compute_gradient_hessian_k2_kernel(
    ...
    __global__ void external_articulation_constraint_time_integrator_update_state_kernel(
        cuda_tool::BufferView<Float>     joint_id_to_delta_theta,
        cuda_tool::CBufferView<Vector2i> joint_id_to_body_ids,
        cuda_tool::CBufferView<Vector6>  joint_id_to_L_basis,
        cuda_tool::CBufferView<Vector6>  joint_id_to_R_basis,
        cuda_tool::CBufferView<Vector12> qs,
...
class ExternalArticulationConstraint final : public InterAffineBodyConstraint
{
  public:
    ...
    {
        int n = (int)joint_joint_id_to_mass.size();
        if(n > 0)
        {
            auto k = external_articulation_constraint_compute_energy_kernel;
            k<<<cuda_tool::best_grid_dim(n, k), cuda_tool::best_block_dim(k), 0, nullptr>>>(
                info.energies(),
                joint_joint_id_to_mass.cview(),
                joint_joint_id_to_joint_ij.cview(),
                joint_id_to_delta_theta_tilde.cview(),
    ...
    void do_compute_gradient_hessian(InterAffineBodyAnimator::GradientHessianInfo& info) override
    {
        ...
        FastSegmentalReduce().reduce(
            ...
            ExternalArticulationConstraintGetKey{joint_joint_id_to_joint_ij.cview()},
            ExternalArticulationConstraintGetValue{
                joint_joint_id_to_mass.cview(),
                joint_joint_id_to_joint_ij.cview(),
                joint_id_to_L_basis.cview(),
                joint_id_to_R_basis.cview(),
        ...
        // Compute Gradient
        int n_gradient = (int)joint_id_to_G_theta.size();
        if(n_gradient > 0)
        {
        ...
        // Compute Hessian
        int n_hessian = (int)joint_joint_id_to_mass.size();
        if(n_hessian > 0)
        {
...
class ExternalArticulationConstraintTimeIntegrator final : public TimeIntegrator
{
  public:
    ...
    void do_update_state(UpdateVelocityInfo& info) override
    {
        ...
        int n = (int)c->joint_id_to_delta_theta.size();
        if(n > 0)
        {
            auto k = external_articulation_constraint_time_integrator_update_state_kernel;
            k<<<cuda_tool::best_grid_dim(n, k), cuda_tool::best_block_dim(k), 0, nullptr>>>(
                c->joint_id_to_delta_theta.view(),
                c->joint_id_to_body_ids.cview(),
                c->joint_id_to_L_basis.cview(),
                c->joint_id_to_R_basis.cview(),
                abd->qs(),
                abd->q_prevs(),
                c->joint_id_to_ref_q_prev_ids.cview(),
                c->ref_q_prevs.view(),
                c->joint_id_to_uid.cview(),
                ExternalArticulationConstituion::RevoluteJointConstitutionUID,
                ExternalArticulationConstituion::PrismaticJointConstitutionUID,
                n);
        }
