...
FusedPcgGraphSignature LinearFusedPCG::current_graph_signature(muda::DenseVectorView<Float> x) const
{
    ...
    signature.alpha                    = resources.alpha.data();
                                            ...
                                            cudaStream_t stream)
{
    ...
    launch_fused_pcg_prepare_alpha(resources.graph_rz_old[current_slot].view(),
                                   resources.graph_pAp[current_slot].view(),
                                   resources.alpha.view(),
                                   d_converged.view(),
                                      ...
                                      SizeT                         max_iter)
{
    ...
    // No terminal state was observed. Keep the compatibility count rather
    // than presenting the last scheduled iteration as a terminal iteration.
    last_terminal_iteration = max_iter;
