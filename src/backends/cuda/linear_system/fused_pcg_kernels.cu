...
namespace
{
    ...
    __global__ void prepare_alpha_kernel(const Float* __restrict__ rz_old,
                                         ...
                                         IndexT iteration_in_chunk)
    {
        ...
        const Float pAp_value = *pAp;

        if(!isfinite(old_value) || !isfinite(pAp_value))
            *status = static_cast<IndexT>(FusedPcgStatus::NonFinite);
        else if(pAp_value <= Float{0.0})
            *status = static_cast<IndexT>(FusedPcgStatus::Breakdown);
        else
            *alpha = old_value / pAp_value;

        if(*status != static_cast<IndexT>(FusedPcgStatus::Running))
        {
            check_state->rz                 = old_value;
                                              ...
                                              IndexT iteration_in_chunk)
    {
        ...
        if(!isfinite(old_value) || !isfinite(new_value))
            *status = static_cast<IndexT>(FusedPcgStatus::NonFinite);
        else if(fabs(new_value) <= params->tolerance)
            *status = static_cast<IndexT>(FusedPcgStatus::Converged);
        else if(old_value == Float{0.0})
            *status = static_cast<IndexT>(FusedPcgStatus::Breakdown);
        else
                                                     ...
                                                     IndexT iteration_in_chunk)
    {
        ...
        Float local_dot = 0.0;
        if(iteration_is_active(params, iteration_in_chunk)
           && *status == static_cast<IndexT>(FusedPcgStatus::Running) && i < vector_size)
        {
            x[i] += *alpha * p[i];
...
void launch_fused_pcg_prepare_alpha(muda::CVarView<Float> rz_old,
                                    ...
                                    cudaStream_t stream)
{
    prepare_alpha_kernel<<<1, 32, 0, stream>>>(rz_old.data(),
                                               pAp.data(),
                                               alpha.data(),
                                               status.data(),
                                               check_state.data(),
