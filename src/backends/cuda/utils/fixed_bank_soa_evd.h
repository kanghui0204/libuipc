...
{
// One thread owns one EVD problem. Matrix coefficient e for lane l is stored
// at shared[e * LanePitch + l], so neighboring lanes access neighboring banks
// instead of spilling a private NxN Eigen workspace to local memory.
...
#pragma unroll
        ...
        // Eigen's self-adjoint matrix-vector product uses a raw-pointer path
        // that assumes innerStride()==1. Express it through coefficients so
        // the shared-memory SoA map keeps its non-unit stride.
#pragma unroll
        for(int row = 0; row < N; ++row)
        {
            if(row < remaining)
...
#pragma unroll
        ...
        // The same applies to Eigen's lower rank-2 update.
#pragma unroll
        for(int col = 0; col < N; ++col)
        {
            if(col < remaining)
                                              ...
                                              Workspace&            q)
{
    ...
    for(int k = start; k < end && z != 0.0; ++k)
    {
        ...
        diag(k) = rotation.c()
                      * (rotation.c() * diag(k) - rotation.s() * subdiag(k))
                  - rotation.s()
                        * (rotation.c() * subdiag(k) - rotation.s() * diag(k + 1));
        ...
        for(int i = start; i < end; ++i)
        {
            ...
            else
            {
                ...
                if(scaled_subdiag * scaled_subdiag
                   <= Eigen::numext::abs(diag(i))
                          + Eigen::numext::abs(diag(i + 1)))
                    subdiag(i) = 0.0;
            }
        }
                                                    ...
                                                    Vector<Float, N>& eigen_values)
{
    ...
    fixed_bank_soa_evd_detail::tridiagonalization_inplace_strided<N>(eigen_vectors,
                                                                      hcoeffs);
    eigen_values = eigen_vectors.diagonal();
    subdiag      = eigen_vectors.template diagonal<-1>();
    Eigen::HouseholderSequence<Workspace, Vector<Float, N - 1>> householder(
        eigen_vectors, hcoeffs);
...
UIPC_DEVICE bool make_spd_fixed_bank_shared_upper_fma(Workspace& eigen_vectors,
                                                       Output&    projected)
{
    Vector<Float, N> eigen_values;
    const bool success =
        selfadjoint_evd_fixed_bank_shared<N>(eigen_vectors, eigen_values);
...
#pragma unroll
            for(int k = 0; k < N; ++k)
            {
                const Float weighted_col =
                    eigen_values(k) * eigen_vectors(col, k);
