...
struct FullGraphFixture
{
    ...
    void launch_iteration(int current_slot, int iteration_in_chunk, int forced_terminal_iteration = 0)
    {
        ...
        spmv.rbk_sym_spmv_dot_pipelined(A.cview(),
                                        ...
                                        triplet_count,
                                        stream);
        launch_fused_pcg_prepare_alpha(rz_old[current_slot].view(),
                                       pAp[current_slot].view(),
                                       alpha.view(),
                                       status.view(),
                                       check_state.view(),
...
{
    static constexpr int AbdBodyCount   = 35;
    static constexpr int FemVertexCount = 1089;
    ...
    ActualDiagonalPreconditionerFixture()
    {
        ...
        for(int body = 0; body < AbdBodyCount; ++body)
            for(int component = 0; component < 12; ++component)
                host_abd[body](component, component) =
                    0.18 + 0.002 * component + 0.0001 * (body % 7);
        ...
        for(int body = 0; body < AbdBodyCount; ++body)
            for(int component = 0; component < 12; ++component)
            {
                const int i  = body * 12 + component;
                initial_z[i] = host_abd[body](component, component) * rhs[i];
            }
        for(int vertex = 0; vertex < FemVertexCount; ++vertex)
            for(int component = 0; component < 3; ++component)
            {
                const int i  = AbdScalarCount + vertex * 3 + component;
                initial_z[i] = host_fem[vertex](component, component) * rhs[i];
            }
        reset();
    }
                           ...
                           bool compare_chunk_local_iteration = true)
{
    ...
    REQUIRE(actual.alpha == Catch::Approx(expected.alpha).margin(1e-12));
...
TEST_CASE("fused_pcg_scalar_status_and_ping_pong", "[cuda][fused_pcg]")
{
    ...
    muda::DeviceVar<Float> pAp{2.0};
    muda::DeviceVar<Float> alpha{0.0};
    ...
    launch_fused_pcg_prepare_alpha(rz_old.view(),
                                   pAp.view(),
                                   alpha.view(),
                                   status.view(),
                                   check_state.view(),
                                   ...
                                   1,
                                   nullptr);
    REQUIRE(copy_var(alpha) == Catch::Approx(2.0));
    ...
    SECTION("non-finite and non-positive curvature are reported")
    {
        status = static_cast<IndexT>(FusedPcgStatus::Running);
        pAp    = std::numeric_limits<Float>::quiet_NaN();
        launch_fused_pcg_prepare_alpha(rz_old.view(),
                                       pAp.view(),
                                       alpha.view(),
                                       status.view(),
                                       ...
                                       4,
                                       nullptr);
        REQUIRE(copy_var(status) == static_cast<IndexT>(FusedPcgStatus::NonFinite));
        ...
        launch_fused_pcg_prepare_alpha(rz_old.view(),
                                       ...
                                       5,
                                       nullptr);
        REQUIRE(copy_var(status) == static_cast<IndexT>(FusedPcgStatus::Breakdown));
    }
}
          ...
          "[cuda][fused_pcg][graph][preconditioner][legacy]")
{
    for(const int iterations : {1, 5, 10})
    {
        DYNAMIC_SECTION("iterations=" << iterations)
        {
            ...
            REQUIRE(fused.alpha == Catch::Approx(legacy.alpha).margin(1e-10));
...
TEST_CASE("fused_pcg_complete_graph_reuses_one_exec_for_1000_reset_chunks",
          "[cuda][fused_pcg][graph][repeat]")
{
    constexpr int    TrialCount = 1000;
    FullGraphFixture fixture{true};
...
TEST_CASE("fused_pcg_terminal_inputs_classify_and_reset_cleanly", "[cuda][fused_pcg][terminal][recovery]")
{
    ...
    const std::array injections{Injection::PApPositiveInf,
                                Injection::PApNegativeInf,
                                Injection::PApNegative,
                                Injection::RzOldNaN,
                                Injection::RzOldPositiveInf,
                                Injection::RzOldNegativeInf,
                                Injection::RzOldZero,
                                Injection::RzNewNaN,
                                Injection::RzNewPositiveInf,
                                Injection::RzNewNegativeInf};

    for(const auto injection : injections)
    {
        DYNAMIC_SECTION("injection=" << static_cast<int>(injection))
        {
            muda::DeviceVar<Float> rz_old{4.0};
            muda::DeviceVar<Float> pAp{2.0};
            muda::DeviceVar<Float> alpha{0.0};
            ...
            const bool inject_new_value = injection == Injection::RzNewNaN
                                          || injection == Injection::RzNewPositiveInf
                                          || injection == Injection::RzNewNegativeInf;

            if(injection == Injection::PApPositiveInf)
                pAp = std::numeric_limits<Float>::infinity();
            else if(injection == Injection::PApNegativeInf)
                pAp = -std::numeric_limits<Float>::infinity();
            else if(injection == Injection::PApNegative)
                pAp = -1.0;
            else if(injection == Injection::RzOldNaN)
                rz_old = std::numeric_limits<Float>::quiet_NaN();
            else if(injection == Injection::RzOldPositiveInf)
                rz_old = std::numeric_limits<Float>::infinity();
            else if(injection == Injection::RzOldNegativeInf)
                rz_old = -std::numeric_limits<Float>::infinity();
            else if(injection == Injection::RzOldZero)
                rz_old = 0.0;
            else if(injection == Injection::RzNewNaN)
                rz_new = std::numeric_limits<Float>::quiet_NaN();
            else if(injection == Injection::RzNewPositiveInf)
                rz_new = std::numeric_limits<Float>::infinity();
            else if(injection == Injection::RzNewNegativeInf)
                rz_new = -std::numeric_limits<Float>::infinity();
            ...
            else
            {
                launch_fused_pcg_prepare_alpha(rz_old.view(),
                                               ...
                                               1,
                                               nullptr);
                if(injection == Injection::RzOldZero)
                {
                    REQUIRE(copy_var(status) == static_cast<IndexT>(FusedPcgStatus::Running));
                    launch_fused_pcg_update_convergence(rz_old.view(),
                                                        rz_new.view(),
            ...
            // A new solve resets all sticky device state. Reuse the same device
            // allocation and prove that a valid scalar iteration runs normally.
            rz_old = 4.0;
            pAp    = 2.0;
            alpha  = 0.0;
            beta   = 0.0;
            rz_new = 1.0;
            status = static_cast<IndexT>(FusedPcgStatus::Running);
            FusedPcgCheckState reset_check;
            reset_check.status = static_cast<IndexT>(FusedPcgStatus::Running);
            reset_check.iteration_in_chunk = 0;
            reset_check.rz                 = 4.0;
            check_state                    = reset_check;
            ...
            REQUIRE(copy_var(alpha) == Catch::Approx(2.0));
            REQUIRE(copy_var(beta) == Catch::Approx(0.25));
            const auto recovered = copy_var(check_state);
            REQUIRE(recovered.status == static_cast<IndexT>(FusedPcgStatus::Running));
            REQUIRE(recovered.iteration_in_chunk == 1);
            REQUIRE(recovered.rz == Catch::Approx(1.0));
        }
    }
}

TEST_CASE("fused_pcg_two_graph_execs_remain_isolated_for_1000_interleaved_runs",
...
{
    constexpr int            BlockRows = 1229;
    constexpr int            MaxCount  = 14347;
    const std::array<int, 5> counts{14331, 14335, 14339, 14343, 14347};
    ...
    for(int trial = 0; trial < TrialCount; ++trial)
    {
        // The first five trials guarantee coverage of every real G2 count;
